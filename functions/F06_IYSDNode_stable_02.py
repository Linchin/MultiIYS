...
10/31/2019 note
stable_02
I will modify the code so it behaves exactly the same
as the old code that worked. The difference that
we spotted is that the first signal should be all 1s.
10/24/2019 note
...
class IYSDNode_normal:
    ...
    def next_time_instant(self, network_signal_history):
        ...
        if self.__time_instant == -1:
            ...
            if last_signal == 1 or combined_signal == 1:
                self.__counter = 1
            else:
                self.__counter += 1
